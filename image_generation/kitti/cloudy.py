...
"""
Build KITTI dataset cloudy weather perturbations (Simplified Version)
"""
import torch
import os
import glob
...
def create_variant(image):
    """Generates a cloudy variant using the global pipe_edit."""
...
def process_image(image_path, src_folder, dest_folder):
    try:
        ...
        cloudy_image = create_variant(image=image_resized)

        target_path = os.path.join(dest_folder, rel_path)
        os.makedirs(os.path.dirname(target_path), exist_ok=True)
        cloudy_image.save(target_path)
...
def main():
    ...
    cloudy_dest_dir = os.path.join(dest_dir, "cloudy")
    os.makedirs(cloudy_dest_dir, exist_ok=True)
    
    # Process specific two directories
    target_folders = ["2011_09_28", "2011_09_29"]
    
    for folder in target_folders:
        src_folder = os.path.join(src_dir, folder)
        dest_folder = os.path.join(cloudy_dest_dir, folder)
        ...
        # Create marker file
        marker_path = os.path.join(dest_folder, ".generated_cloudy_v1")
        try:
            with open(marker_path, "w") as f:
                device_name = torch.cuda.get_device_name(torch.cuda.current_device())
                f.write(f"This directory contains cloudy variants generated on {device_name}.")
        except Exception as e:
            print(f"Warning: Could not write marker file {marker_path}: {e}")
    
    print("cloudy variant dataset build complete!")
