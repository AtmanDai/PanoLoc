...
"""
Build MGL dataset with snowy perturbations using distributed processing across multiple nodes.
...
dest_dir = "/hkfs/work/workspace/scratch/tj3409-rdaionet/OrienterNet/datasets/MGL_snowy"

PROMPT_CONFIG = {
    "prompt": "change the background into a snowy day, add visible falling snowflakes",
    "seed": 42,
...
def create_variant(pipe, input_image, device, prompt_config):
    """Generates a snowy variant of a single image using the provided pipeline."""
...
def process_chunk(local_gpu_rank: int, local_world_size: int, node_image_files: List[str], 
    ...
    for image_path in tqdm(gpu_image_chunk, desc=f"Node{node_rank}-GPU{local_gpu_rank}", position=node_rank*4+local_gpu_rank):
        try:
            ...
            snowy_image = create_variant(pipe, input_image, device, prompt_config)

            os.makedirs(os.path.dirname(target_path), exist_ok=True)
            snowy_image.save(target_path)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Distributed MGL snowy variant generation.")
    ...
    if args.node_rank == 0:
        marker_path = os.path.join(dest_folder, ".generated_snowy_distributed")
        try:
            with open(marker_path, "w") as f:
                timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                f.write(f"Distributed snowy variants generated at {timestamp}\n")
