...
def train(cfg: DictConfig, job_id: Optional[int] = None):
    ...
    if init_checkpoint_path is not None:
        ...
        checkpoint_state_dict = torch.load(Path(init_checkpoint_path), map_location="cpu", weights_only=False)['state_dict']

        # Combine the pre trained SAM2 Encoder with the pre-trained model except the image encoder.
        # The hybrid model should contain the best knowledge learned in SAM2 image encoding,
        # and in pre-trained map encoding, bev-net.
        hybrid_model = model.model
        onet_sam2_state_dict = hybrid_model.state_dict()

        # Create a new state_dict which contains the correspond weights we need from OrienterNet_MGL
        hybrid_state_to_load = {}
        for key, value in checkpoint_state_dict.items():
            if not key.startswith("model.image_encoder"):
                if key in onet_sam2_state_dict and onet_sam2_state_dict[key].shape == value.shape:
                    hybrid_state_to_load[key] = value
        
        # Set strict=False to allow partially state_dict loading
        hybrid_model.load_state_dict(hybrid_state_to_load, strict=False)
    else:
        model = GenericModule(cfg)
    if rank == 0:
        logger.info("Network:\n%s", model.model)
