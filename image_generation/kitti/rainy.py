...
"""
Build KITTI dataset rainy weather perturbations (Simplified Version)
"""
import torch
import os
import glob
...
def create_variant(image):
    """Generates a rainy variant using the global pipe_edit."""
...
def process_image(image_path, src_folder, dest_folder):
    ...
    try:
        ...
        rainy_image = create_variant(image=image_resized)
...
def main():
    ...
    rainy_dest_dir = os.path.join(dest_dir, "rainy")
    os.makedirs(rainy_dest_dir, exist_ok=True)
    ...
    for folder in target_folders:
        ...
        dest_folder = os.path.join(rainy_dest_dir, folder)
        ...
        # Create marker file
        marker_path = os.path.join(dest_folder, ".generated_rainy_v1")
        try:
            with open(marker_path, "w") as f:
                device_name = torch.cuda.get_device_name(torch.cuda.current_device())
                f.write(f"This directory contains rainy variants generated on {device_name}.")
        except Exception as e:
            print(f"Warning: Could not write marker file {marker_path}: {e}")
    
    print("Rainy variant dataset build complete!")
