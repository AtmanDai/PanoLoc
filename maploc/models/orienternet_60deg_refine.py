...
from .metrics_v1gt_60deg import AngleError, AngleRecall, Location2DError, Location2DRecall
from .voting import (
    TemplateSampler,
    argmax_xyr,
...
class OrienterNet(BaseModel):
    ...
    def _init(self, conf):
        ...
        grid_xz_pano = make_pano_grid(conf.z_max, conf.x_max, ppm)
    ...
    def _is_panorama_batch(self, data, num_views):
        """
        Check if the batch contains panorama views (groups of num_views).
        """
    ...
    def _rotate_bev_around_camera(self, features, angle_deg):
        ...
        padding = (W, W, H, H)         # F.pad:(left, right, top, bottom)
    ...
    def _stitch_panorama_bev(self, f_bev, valid_bev):
        ...
        B, C, H, W = f_bev.shape
        device = f_bev.device
        assert B % self.num_views == 0, "Batch size must be a multiple of 3 for panorama processing"
        num_panoramas = B // self.num_views
        num_pairs = num_panoramas * 3 # The number of views after concatenation
        part_num = 2 # The number of views to be concatenated as a pair

        f_bev_stitched = torch.zeros(
            (num_pairs, C, H * 3, W * 3), device=device, dtype=f_bev.dtype
        )
        valid_stitched = torch.zeros(
            (num_pairs, H * 3, W * 3), device=device, dtype=torch.bool
        )

        for i in range(num_pairs):
            start_idx = i * part_num

            f_v1, v_v1 = f_bev[start_idx], valid_bev[start_idx]
            f_v2, v_v2 = f_bev[start_idx + 1], valid_bev[start_idx + 1]
            
            # Concatanate v1 and v2 into a 120 deg single view
            f_v1_rot = self._rotate_bev_around_camera(f_v1, -30)
            v_v1_rot = (
                self._rotate_bev_around_camera(
                    v_v1.unsqueeze(0).float(), -30
                )
                .squeeze(0) > 0.5
            )
            
            f_v2_rot = self._rotate_bev_around_camera(f_v2, 30)
            v_v2_rot = (
                self._rotate_bev_around_camera(
                    v_v2.unsqueeze(0).float(), 30
                )
                .squeeze(0) > 0.5
            )
            ...
            denominator = (v_v1_rot.float() + v_v2_rot.float())
            ...
            valid_stitched[i] = v_v1_rot | v_v2_rot
    ...
    def _forward(self, data):
        ...
        if is_panorama and self.conf.fusion_strategy == "view1_yaw_prior":
            ...
            if "log_prior" in pred_map:
                pred_map["log_prior"][0] = pred_map["log_prior"][0][::2]
            ...
            # Stitch the 2 views into a 120 deg view
        ...
        f_bev_v1 = features_bev_individual[::2]
        valid_bev_v1 = valid_bev_individual[::2]
        confidence_bev_v1 = confidence_bev_individual[::2] if confidence_bev_individual is not None else None
        ...
        if "map_mask" in data:
            map_mask = data["map_mask"][::2] if is_panorama else data["map_mask"]
            scores.masked_fill_(~map_mask[..., None], -np.inf)
        if "yaw_prior" in data:
            yaw_prior = data["yaw_prior"][::2] if is_panorama else data["yaw_prior"]
    ...
    def loss(self, pred, data):
        ...
        if pred.get("is_panorama", False):
            ...
            xy_gt = data["uv"][::2]
            yaw_gt_1 = data["roll_pitch_yaw"][::2, -1]
            yaw_gt_2 = data["roll_pitch_yaw"][1::2, -1]
            yaw_gt = self._angle_mid(yaw_gt_1, yaw_gt_2)
