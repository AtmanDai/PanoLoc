...
# New Depth Head Module
class DepthHead(nn.Module):
    def __init__(self, in_channels):
        super().__init__()
        # A simple head: Conv -> ReLU -> Conv to output a single channel depth map
        self.head = nn.Sequential(
            nn.Conv2d(in_channels, in_channels // 2, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels // 2, 1, kernel_size=1, padding=0),
            nn.Sigmoid() # Use Sigmoid to constrain output, assuming depth is normalized
        )
...
class FeatureExtractor(BaseModel):
    default_conf = {
        ...
        "predict_depth": True, # Add a flag to control depth prediction
    }
    ...
    def _init(self, conf):
        ...
        # Decoder for localization features
        ...
        # New Depth Head
        if self.conf.predict_depth:
            self.depth_head = DepthHead(in_channels=conf.output_dim)
    ...
    def _forward(self, data):
        ...
        loc_features = self.decoder(skip_features)
        
        pred = {"feature_maps": [loc_features], "skip_features": skip_features}
        
        # Predict depth map if the head exists
        if self.conf.predict_depth:
            depth_map = self.depth_head(loc_features)
            pred["depth_map"] = depth_map
