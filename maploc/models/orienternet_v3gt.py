...
from .metrics_v3gt import AngleError, AngleRecall, Location2DError, Location2DRecall
from .voting import (
    TemplateSampler,
    argmax_xyr,
...
class OrienterNet(BaseModel):
    ...
    def _stitch_panorama_bev(self, f_bev, valid_bev):
        ...
        for i in range(num_panoramas):
            ...
            f_v1_rot = self._rotate_bev_around_camera(f_v1, 120)
            v_v1_rot = (
                self._rotate_bev_around_camera(
                    v_v1.unsqueeze(0).float(), 120
                )
                .squeeze(0) > 0.5
            )
            ...
            f_v3_rot = self._rotate_bev_around_camera(f_v3, 0)
            v_v3_rot = (
                self._rotate_bev_around_camera(
                    v_v3.unsqueeze(0).float(), 0
                )
                .squeeze(0) > 0.5
            )
    ...
    def _forward(self, data):
        ...
        if is_panorama:
            ...
            if "log_prior" in pred_map:
                pred_map["log_prior"][0] = pred_map["log_prior"][0][2::3]
        ...
        if "map_mask" in data:
            map_mask = data["map_mask"][2::3] if is_panorama else data["map_mask"]
            scores.masked_fill_(~map_mask[..., None], -np.inf)
        if "yaw_prior" in data:
            yaw_prior = data["yaw_prior"][2::3] if is_panorama else data["yaw_prior"]
    ...
    def loss(self, pred, data):
        ...
        if pred.get("is_panorama", False):
            ...
            xy_gt = data["uv"][2::3]
            yaw_gt = data["roll_pitch_yaw"][2::3, -1]
