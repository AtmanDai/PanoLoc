...
from .new_metrics import AngleError, AngleRecall, Location2DError, Location2DRecall
from .voting import (
    TemplateSampler,
    argmax_xyr,
...
class OrienterNet(BaseModel):
    ...
    def _stitch_panorama_bev(self, f_bev, valid_bev):
        ...
        for i in range(num_panoramas):
            ...
            # View 2 (clockwise +120 deg yaw): Rotate its content by -120 deg to align.
            ...
            # View 3 (+240 deg yaw): Rotate its content by -240 deg (i.e., +120 deg) to align.
    ...
    def _forward(self, data):
        ...
        scores_single_view = self.exhaustive_voting(
            f_bev_individual, f_map, valid_bev_individual, confidence_bev_individual, is_panorama=False
        )
        ...
        return {
            **pred,
            ...
            "features_bev_individual": f_bev_individual,
    ...
    def loss(self, pred, data):
        ...
        if self.conf.do_label_smoothing:
            ...
            yaw_candidates = [yaw_gt_v1, yaw_gt_v2, yaw_gt_v3]
            nll_losses = [nll_loss_xyr(pred["log_probs"], xy_gt, yaw) for yaw in yaw_candidates]
            stacked_losses = torch.stack(nll_losses, dim=0)
            min_nll, min_indices = torch.min(stacked_losses, dim=0)
            nll = min_nll
            stacked_yaws = torch.stack(yaw_candidates, dim=0)
            idx = min_indices.unsqueeze(0)
            yaw_gt = torch.gather(stacked_yaws, 0, idx).squeeze(0)
