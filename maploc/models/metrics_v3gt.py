...
class AngleRecall(torchmetrics.MeanMetric):
    ...
    def update(self, pred, data):
        ...
        # --- FIX END ---
        error = angle_error(pred[self.key], yaw_gt)
...
class AngleError(MeanMetricWithRecall):
    ...
    def update(self, pred, data):
        ...
        if is_panorama:
            yaw_gt = data["roll_pitch_yaw"][2::3, -1]
        # --- FIX END ---
        value = angle_error(pred[self.key], yaw_gt)
        if value.numel():
...
class LateralLongitudinalError(MeanMetricWithRecall):
    ...
    def update(self, pred, data):
        ...
        if is_panorama:
            uv_gt = uv_gt[2::3]
            yaw = deg2rad(data["roll_pitch_yaw"][2::3, -1])
        shift = (pred[self.key] - uv_gt) * yaw.new_tensor([-1, 1])
