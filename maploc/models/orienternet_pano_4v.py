...
class OrienterNet(BaseModel):
    ...
    '''
    def _create_blending_weights(self, H, W, device):
        """
        Create a weighted mask, the weight decline from the center with 1 to edge with 0
        """
        y, x = torch.meshgrid(
        ...
        angle = torch.atan2(y, x) # 角度范围是 [-pi, pi]
        
        # Within +/- 60 degree (pi/3) decline from 1 to 0 
        center_angle = torch.tensor(np.pi / 2, device=device)
        
        angular_distance = torch.abs(angle - center_angle)
        
        half_angle_width = torch.tensor(np.pi / 3, device=device)
        
        # 在边界处稍微扩大一点范围 (e.g., 61度) 来确保平滑
        weights = 1.0 - torch.clamp(angular_distance / (half_angle_width * 1.02), 0, 1)
    ...
    def _rotate_bev_around_camera(self, features, angle_deg):
        ...
        padding = (0, 0, 0, H)         # F.pad:(left, right, top, bottom)
    ...
    def _stitch_panorama_bev(self, f_bev, valid_bev):
        """
        Stitches BEV features and masks from n panorama views by rotating them
        around the camera position to form a single, coherent 360-degree BEV.

        Args:
            f_bev: [B, C, H, W] - BEV features for all n views.
        ...
        B, C, H, W = f_bev.shape
        device = f_bev.device
        assert B % self.num_views == 0, "Batch size must be a multiple of n for panorama processing"
        num_panoramas = B // self.num_views

        f_bev_stitched = torch.zeros(
            (num_panoramas, C, 2*H, W), device=device, dtype=f_bev.dtype
        )
        valid_stitched = torch.zeros(
            (num_panoramas, 2*H, W), device=device, dtype=torch.bool
        )

        for i in range(num_panoramas):
            start_idx = i * self.num_views
            ...
            f_v4, v_v4 = f_bev[start_idx + 3], valid_bev[start_idx + 3]
            
            # View 1: front (0 deg yaw): The reference view, no rotation needed.
            ...
            # View 2: left, Rotate its content anti-clockwisely by 90 deg to align.
            f_v2_rot = self._rotate_bev_around_camera(f_v2, 90)
            v_v2_rot = (
                self._rotate_bev_around_camera(
                    v_v2.unsqueeze(0).float(), 90
                )
                .squeeze(0) > 0.5
            )

            # View 3: back, Rotate its content anti-clockwisely by 180 deg to align.
            f_v3_rot = self._rotate_bev_around_camera(f_v3, 180)
            v_v3_rot = (
                self._rotate_bev_around_camera(
                    v_v3.unsqueeze(0).float(), 180
                )
                .squeeze(0) > 0.5
            )
            
            # View 4: right, Rotate its content anti-clockwisely by 270 deg to align.
            f_v4_rot = self._rotate_bev_around_camera(f_v4, -90)
            v_v4_rot = (
                self._rotate_bev_around_camera(
                    v_v4.unsqueeze(0).float(), -90
                )
                .squeeze(0) > 0.5
            )
            ...
            f_v4_masked = torch.where(v_v4_rot.unsqueeze(0), f_v4_rot, 0.0)
            ...
            numerator = (f_v1_masked * v_v1_rot.unsqueeze(0).float() +
                     ...
                     f_v4_masked * v_v4_rot.unsqueeze(0).float())
            
            denominator = (v_v1_rot.float() + v_v2_rot.float() + v_v3_rot.float() + v_v4_rot.float())
            ...
            valid_stitched[i] = v_v1_rot | v_v2_rot | v_v3_rot | v_v4_rot
    ...
    def _forward(self, data):
        ...
        if is_panorama:
            ...
            f_map = f_map[::self.num_views]
            if "log_prior" in pred_map:
                pred_map["log_prior"][0] = pred_map["log_prior"][0][::self.num_views]
            ...
            # Stitch the n views into a full panorama
        ...
        if "map_mask" in data:
            map_mask = data["map_mask"][::self.num_views] if is_panorama else data["map_mask"]
            scores.masked_fill_(~map_mask[..., None], -np.inf)
        if "yaw_prior" in data:
            yaw_prior = data["yaw_prior"][::self.num_views] if is_panorama else data["yaw_prior"]
    ...
    def loss(self, pred, data):
        ...
        if pred.get("is_panorama", False):
            ...
            xy_gt = data["uv"][::self.num_views]
            yaw_gt_v1 = data["roll_pitch_yaw"][::self.num_views, -1]
            yaw_gt_v2 = data["roll_pitch_yaw"][1::self.num_views,-1]
            yaw_gt_v3 = data["roll_pitch_yaw"][2::self.num_views,-1]
            yaw_gt_v4 = data["roll_pitch_yaw"][3::self.num_views,-1]
        ...
        if self.conf.do_label_smoothing:
            ...
            yaw_candidates = [yaw_gt_v1, yaw_gt_v2, yaw_gt_v3, yaw_gt_v4]
            nll_losses = [nll_loss_xyr(pred["log_probs"], xy_gt, yaw) for yaw in yaw_candidates]
            min_idx = nll_losses.index(min(nll_losses))
            nll = nll_losses[min_idx]
            yaw_gt = yaw_candidates[min_idx]
