...
class OrienterNet(BaseModel):
    ...
    def _stitch_panorama_bev(self, f_bev, valid_bev):
        """
        Stitches BEV features and masks from a single view by rotating it
        ...
        Returns:
            f_bev_stitched: [B, C, H, W] - Stitched features for each panorama.
            valid_stitched: [B, H, W] - Combined valid mask for each panorama.
        """
        B, C, H, W = f_bev.shape
        device = f_bev.device
        num_batch = B
        ...
        f_bev_stitched = torch.zeros(
            (num_batch, C, 4*H, W), device=device, dtype=f_bev.dtype
        )
        valid_stitched = torch.zeros(
            (num_batch, 4*H, W), device=device, dtype=torch.bool
        )

        for i in range(num_batch):

            f_v1, v_v1 = f_bev[i], valid_bev[i]
            f_v2, v_v2 = f_bev[i], valid_bev[i]
            f_v3, v_v3 = f_bev[i], valid_bev[i]
    ...
    def _forward(self, data):
        ...
        if is_panorama:
            ...
            if "log_prior" in pred_map:
                pred_map["log_prior"][0] = pred_map["log_prior"][0]
