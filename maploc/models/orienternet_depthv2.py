...
class OrienterNet(BaseModel):
    default_conf = {
        ...
        "sigma_xy": 1,
        "sigma_r": 2,
        "depth_loss_weight": 0.1,  # Weight for depth supervision
        # depcreated
    ...
    def _init(self, conf):
        ...
        depth_model.load_state_dict(torch.load(f'Depth_Anything_V2/checkpoints/depth_anything_v2_{depth_encoder}.pth', map_location='cpu'))
        self.depth_model = depth_model.to('cuda').eval()
    ...
    def _forward(self, data):
        ...
        # Extract image features and depth map from the encoder.
        image_pred = self.image_encoder(data)
        f_image = image_pred["feature_maps"][0]
        pred["depth_map_predicted"] = image_pred["depth_map"]
        ...
        # Get supervisor depth from DepthAnythingV2.
        image_resized = F.interpolate(
            data["image"], size=(518, 518), mode='bilinear', align_corners=False
        )
        with torch.no_grad():
            image_depth_supervisor = self.depth_model(image_resized)
        pred['depth_supervisor'] = image_depth_supervisor

        # Map to the Cartesian BEV.
        with torch.autocast("cuda", enabled=False):
        ...
        features_bev_individual = f_bev.clone()
        valid_bev_individual = valid_bev.clone()
        confidence_bev_individual = confidence_bev.clone() if confidence_bev is not None else None
    ...
    def loss(self, pred, data):
        ...
        depth_pred = pred["depth_map_predicted"]     # [B, 1, H, W]
        depth_supervisor = pred["depth_supervisor"]  # [B, H_super, W_super]

        # Resize supervisor depth to match prediction size
        B, _, H, W = depth_pred.shape
        depth_supervisor_resized = F.interpolate(
            depth_supervisor.unsqueeze(1), size=(H, W), mode='bilinear', align_corners=False
        ) # [B, 1, H, W]

        
        depth_loss = F.mse_loss(depth_pred, depth_supervisor_resized)
        weighted_depth_loss = self.conf.depth_loss_weight * depth_loss
        
        total_loss = nll + weighted_depth_loss
        
        loss = {"total": total_loss, "nll": nll, "depth_loss": weighted_depth_loss}
